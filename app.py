...
from fastapi import FastAPI, Request  # Added
            ...
            with gr.Column():
                ...
                with gr.Row():
                    with gr.Tabs():
                        ...
                        # --- Added Rig from URL Tab ---
                        with gr.Tab("Rig from URL"):
                            with gr.Column():
                                url_input = gr.Textbox(
                                    label="Model URL (e.g., from Render.com, must be public .glb/.fbx)",
                                    placeholder="https://example.com/path/to/your/model.glb"
                                )
                                # Changed output to File for download, as Model3D can't display external URLs easily
                                rigged_output = gr.File(label="Rigged Model (Download Link / File)")
                                rig_btn = gr.Button("Rig Model from URL")

                                gr.Markdown("---")
                                gr.Markdown("**(Test with a pre-defined URL)**")
                                # Keep test button as before
                                test_btn = gr.Button("Test Render.com Model (punk_test.glb)")

                                # Rig button action
                                rig_btn.click(
                                    fn=rig_from_url,
                                    inputs=url_input,
                                    outputs=rigged_output,
                                    show_progress="full" # Show progress as it might take time
                                )

                                # Test button action
                                def test_rig():
                                    test_url = "https://viverse-backend.onrender.com/models/meshy/test_avatar/punk_test.glb"
                                    print(f"Testing rig_from_url with: {test_url}")
                                    return rig_from_url(test_url)
                                test_btn.click(
                                    fn=test_rig,
                                    inputs=None, # No direct input from UI for test
                                    outputs=rigged_output,
                                    show_progress="full"
                                )
                        # --- End of Added Tab ---
...
if __name__ == "__main__":
    ...
    # Brief pause to allow FastAPI server to start - adjust if needed
    time.sleep(2)

    # Create the Gradio interface
    print("Initializing Gradio blocks...")
    demo = init_blocks()
    print("Gradio blocks initialized.")

    # Launch the Gradio app with queue enabled
    print("Launching Gradio App...")
    demo.queue(default_concurrency_limit=1).launch( # Use queue as in app.py
        server_name="0.0.0.0",
        server_port=7860,
        show_error=True,
        # allowed_paths=["."], # Removed as per app.py
        # ssr_mode=False # Removed as per app.py
    )
    print("Gradio App launched.")

    # Optional: Keep main thread alive if needed, though Gradio launch usually does this
    # api_thread.join() # This would block until API server stops, likely not desired here
